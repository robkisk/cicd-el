...
```bash
conda create -n cicd_el python=3.9
conda activate cicd_el
```
...
```
This command will create your personal repository under `/Repos/<username>/cicd_el`.
...
- To trigger the release pipeline, get the current version from the `cicd_el/__init__.py` file and tag the current code version:
```
